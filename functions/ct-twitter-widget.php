...
function widget( $args, $instance ) {
	...
	$consumer_key = $ct_data['ct_consumer_key'];
	$consumer_secret = $ct_data['ct_consumer_secret'];
	$user_token = $ct_data['ct_user_token'];
	$user_secret = $ct_data['ct_user_secret'];
	...
	if($diff >= $crt || empty($ct_twitter_last_cache_time)){
		...
		update_option('tp_twitter_plugin_tweets'.$instance['username'], $tweets_array_serial);
...
if ( !function_exists( 'ct_relative_time' ) ) {
	...
	$tp_twitter_plugin_tweets = maybe_unserialize( utf8_decode ( get_option( 'tp_twitter_plugin_tweets'.$instance['username'] ) ) );
