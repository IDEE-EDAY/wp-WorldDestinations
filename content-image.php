		...
		<header class="entry-header">
			...
			<?php elseif ( $show_title == '1' ) : // Show title block ?>
				<h1 class="entry-title">
					<?php if ( $post_font_color == '#363636' ) : ?>
					<a href="<?php if (!empty($mb_external_url)):echo esc_url( $mb_external_url ); else: the_permalink(); endif; ?>" <?php if (!empty($mb_external_url)):echo 'target="_blank"'; endif; ?> title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'color-theme-framework' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"><?php the_title(); ?></a>
					<?php else : ?>
					<a href="<?php if (!empty($mb_external_url)):echo esc_url( $mb_external_url ); else: the_permalink(); endif; ?>" <?php if (!empty($mb_external_url)):echo 'target="_blank"'; endif; ?> title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'color-theme-framework' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark" style="color: <?php echo $post_font_color; ?>"><?php the_title(); ?></a>
					<?php endif; ?>
				</h1>
			<?php else : // Show meta entry-title for Rich Snip Tool  ?>
				<h1 class="entry-title" style="display: none;"><a href="<?php if (!empty($mb_external_url)):echo esc_url( $mb_external_url ); else: the_permalink(); endif; ?>" <?php if (!empty($mb_external_url)):echo 'target="_blank"'; endif; ?> title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'color-theme-framework' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark" style="color: <?php echo $post_font_color; ?>"><?php the_title(); ?></a></h1>
			<?php endif; // is_single() ?>
		...
		<?php if ( has_post_thumbnail() ) : ?>
			...
			<?php elseif ( !is_single() ): ?>
				<div class="entry-thumb">
					...
					?>
					<a href="<?php if (!empty($mb_external_url)):echo esc_url( $mb_external_url ); else: echo the_permalink(); endif; ?>" <?php if (!empty($mb_external_url)):echo 'target="_blank"'; endif; ?>><img src="<?php echo $thumb_image_url[0]; ?>" title="<?php the_title(); ?>" alt="<?php the_title(); ?>" /></a>
				</div> <!-- .entry-thumb -->
			<?php endif; ?>
		<?php endif; ?>
